...
// create 3 new pizza orders, using the arrays of pizzas and customers that have been created in PizzaShop.java
	...
	//gets all customers from pizzashop @ line 55
	
	Pizza [] allPizzas = pizzaShop.getPizzas(); 
	//gets all pizza types from pizzashop @ line 55

// create order 1
	
	Customer orderCustomer1 = customers[1]; 
	//the customer at array index 1, NOT the first array item (which would be [0]
	
	Pizza [] pizzaOrder1 = new Pizza[2]; 
	//this is the array length, not the pizza type, i.e. number of pizzas in the order
	
	pizzaOrder1[0] = allPizzas[1]; 
	//first pizza in the Pizza array. allPizzas index value is the pizza type, from line 27
	
	pizzaOrder1[1] = allPizzas[2]; 
	//second pizza in the array
	
	pizzaShop.addOrder(orderCustomer1, pizzaOrder1); 
	//passed to addOrder method in pizzashop @ line 48
