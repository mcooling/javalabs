 ...
 * This class should extend Customer and add a new field, private String emailAddress;
 * Constructor for InternetCustomer takes the same arguments as Customer, plus an extra argument for email address. 
 * Pass the arguments relevant to customer to the superclass
 * Make a call to the constructor in Customer using the keyword ‘super’ (as shown in the lecture) 
 */
...
public class InternetCustomer extends Customer
{
    ...
    {
	String output = super.toString() + " email address: " + emailAddress; 
	//need to add the new emailAddress variable, as the super.toString method is only returning the name and 	address from superclass
	return output;
    }
